  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Se confirmação de e-mail estiver desabilitada, session vem direto
        if (data.session) {
          router.push('/dashboard');
          return;
        }

        // Caso o email já exista, o Supabase pode retornar user sem identities
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          <button
            disabled={loading}
            type="submit"
            className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg transition-colors flex justify-center"
          >
