...
const PricingPage: React.FC = () => {
    ...
    const targetHourlyRate = (settings.monthly_goal && billableHours > 0) 
        ? (settings.monthly_goal / billableHours) / (1 - settings.tax_percent/100)
        : 0;
                    ...
                    <div className="space-y-6 print:hidden">
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <div><label className="block text-sm font-medium text-slate-700 mb-1">Horas Estimadas de Trabalho</label><input type="number" className="w-full border border-slate-300 p-2 rounded-lg bg-white text-slate-900" value={calcHours} onChange={e => setCalcHours(e.target.value)} /><div className="mt-2 flex gap-3 text-xs"><div className="bg-slate-100 p-1.5 rounded text-slate-600">Mínimo (Custo): <strong>R$ {minHourlyRateTaxed.toFixed(2)}/h</strong></div>{targetHourlyRate > 0 && (<div className="bg-emerald-50 p-1.5 rounded text-emerald-700">Alvo (Meta): <strong>R$ {targetHourlyRate.toFixed(2)}/h</strong></div>)}</div></div>
